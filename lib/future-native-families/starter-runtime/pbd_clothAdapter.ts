import { getDefaultPbdClothConfig, type PbdClothNormalizedConfig } from './pbd_clothSchema';
import { createPbdClothRuntimeState, type PbdClothRuntimeState } from './pbd_clothSolver';

export type PbdClothInputConfig = Partial<PbdClothNormalizedConfig>;

export function normalizePbdClothConfig(input?: PbdClothInputConfig): PbdClothNormalizedConfig {
  const defaults = getDefaultPbdClothConfig();
  return {
    ...
    pinMode: input?.pinMode ?? defaults.pinMode,
    pinGroupWidth: Math.max(1, Math.floor(input?.pinGroupWidth ?? defaults.pinGroupWidth)),
...
export function createPbdClothRuntimeFromInput(input?: PbdClothInputConfig): PbdClothRuntimeState {
  return createPbdClothRuntimeState(normalizePbdClothConfig(input));
}
